...
export default function RespiteCareComponent() {
    ...
    async function fetchData() {
      try {
        const response = await fetch(
          "https://admin.interimhc.com/api/chico-respite-cares?populate[maincontent][populate]=*&populate[seo]=*"
        );
  ...
  const renderDescription = (desc) => {
    if (desc.type === "list") {
      return renderList(desc.children); // Only render list here and skip further processing
    }
  ...
  {data.maincontent[2]?.description.map((desc, index) => {
    if (desc.type === "list") {
      return renderList(desc.children); // Only call renderList for list items
    } else {
      return (
