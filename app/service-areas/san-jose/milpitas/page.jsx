...
export default function MilpitasPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch("http://127.0.0.1:1337/api/milpitas-californias?populate[maincontent][populate]=*");
                    ...
                    <Row className="py-5">
                        <Col md={6} style={{ paddingRight: '25px' }} className="px-0">
                            {content[1]?.image?.data && (
                                <Image src={`http://127.0.0.1:1337${content[1].image.data.attributes.url}`} width={626} height={525} alt="Milpitas Image 1" />
                            )}
                        </Col>
                    ...
                    <Container className="section4subcity py-5">
                        <Row>
                            ...
                            <Col md={6} className="px-0">
                                {content[2]?.img?.data && (
                                    <Image src={`http://127.0.0.1:1337${content[2].img.data.attributes.url}`} width={626} height={525} alt="Milpitas Image 2" />
                                )}
                            </Col>
                        </Row>
                    </Container>
                    ...
                    <Row>
                        <Col md={6} style={{ paddingRight: '25px' }}>
                            {content[3]?.image?.data && (
                                <Image src={`http://127.0.0.1:1337${content[3].image.data.attributes.url}`} width={595} height={780} alt="Milpitas Image 3" />
                            )}
                        </Col>
                        <Col md={6} style={{ paddingLeft: '25px' }}>
