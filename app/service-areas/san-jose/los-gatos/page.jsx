...
export default function LosGatosPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const response = await fetch("http://127.0.0.1:1337/api/los-gatos-californias?populate[maincontent][populate]=*");
                ...
                <Container fluid>
                    <Row className="py-5">
                        <Col md={6} style={{ paddingRight: '25px' }} className="px-0">
                            <Image
                                src={`http://127.0.0.1:1337${content[1]?.image?.data?.attributes?.url}`}
                                width={626}
                                height={525}
                    ...
                    <Container className="section4subcity py-5">
                        <Row>
                            ...
                            <Col md={6} className="px-0">
                                <Image
                                    src={`http://127.0.0.1:1337${content[2]?.img?.data?.attributes?.url}`}
                                    width={626}
                                    height={525}
                ...
                <Container>
                    <Row>
                        <Col md={6} style={{ paddingRight: '25px' }}>
                            <Image
                                src={`http://127.0.0.1:1337${content[3]?.image?.data?.attributes?.url}`}
                                width={595}
                                height={780}
