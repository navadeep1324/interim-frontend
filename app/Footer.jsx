...
export default function Footer() {
    ...
    useEffect(() => {
        const fetchFooterData = async () => {
            try {
                const response = await axios.get("http://127.0.0.1:1337/api/homes?populate[maincontent][populate]=*");
                ...
                <Container>
                    ...
                    <Row style={{ paddingTop: '3em' }}>
                        {footerData?.maincontent?.filter(content => content.__component === "layout.begin-your-senior-s-journey-with-us").map((item, index) => (
                            <Col md="4" key={index}>
                                <Image
                                    src={`http://127.0.0.1:1337${item?.image?.data?.attributes?.url}`}
                ...
                <Container>
                    <Row>
                        {footerData?.maincontent?.filter(content => content.__component === "components.home-page-awards").map((award, index) => (
                            <Col md={4} key={index}>
                                <Image
                                    src={`http://127.0.0.1:1337${award?.Img?.data?.attributes?.url}`}
